> Archived from docs/NOW.md on 2026-06-07 · Sprint: Real Delta-T Sprint
...
All tasks complete. ODE implementation corrected mid-sprint: batch-mean delta_t (v1,
run ode_realdelta_seed42) replaced with grouped per-example odeint (v2, run
ode_realdelta_v2_seed42) — the batch-mean version made ODE predictions batch-composition-
dependent and withheld per-example timing from the ODE while recurrents got exact gaps.

Final corrected results (19 test eyes, seed=42):
...
ODE improves with real timing; recurrents degrade. Directional evidence for dynamics
thesis. CLAIMS.md updated with "DIRECTIONAL EVIDENCE" section (not promoted to CAN CLAIM
— 19 eyes insufficient for statistical certainty). Decision #11 updated.
